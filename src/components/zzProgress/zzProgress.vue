...
export default {
    ...
    methods: {
        ...
        proMove(e) {
            ...
            let playWidth = Math.min(this.$refs.zzProgress.clientWidth, Math.max(0, this.move.left + dist))
        ...
        proClick(e) {
            ...
            //  e.clientX-rect.left
            //  偏移位置控制在进度条宽度内
            let playWidth = Math.min(this.$refs.zzProgress.clientWidth, Math.max(0, e.clientX - rect.left))
            // 设置滚动条位置
            this.progressMove(playWidth)
            // 设置播放时间
            this.setPercent(playWidth)
